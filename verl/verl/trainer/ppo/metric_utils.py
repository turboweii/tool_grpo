...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # ============================================================================
    # W4 Quick-Win A: group reward distribution metrics
    # 用于验证 P1 "双向死锁" 假说：group 内 reward 是否饱和（全 0 或全 1）
    # ============================================================================
    score_std = torch.std(non_aborted_sequence_score).detach().item()
    all_zero_frac = (non_aborted_sequence_score == 0).float().mean().detach().item()
    all_one_frac = (non_aborted_sequence_score == 1).float().mean().detach().item()

    metrics["critic/score/std"] = score_std
    metrics["critic/score/all_zero_frac"] = all_zero_frac
    metrics["critic/score/all_one_frac"] = all_one_frac
    ...
    # [W5] per-turn reasoning tokens: monitor reasoning degradation during training
    if "reasoning_tokens_per_turn" in batch.non_tensor_batch:
        rtpt = batch.non_tensor_batch["reasoning_tokens_per_turn"]
        all_tokens = []
        per_sample_avg = []
        for sample in rtpt:
            if sample and len(sample) > 0:
                all_tokens.extend(sample)
                per_sample_avg.append(float(np.mean(sample)))
        if all_tokens:
            metrics["reasoning_tokens_per_turn/mean"] = float(np.mean(all_tokens))
            metrics["reasoning_tokens_per_turn/max"] = int(np.max(all_tokens))
            metrics["reasoning_tokens_per_turn/min"] = int(np.min(all_tokens))
        if per_sample_avg:
            metrics["reasoning_tokens_per_turn/sample_mean"] = float(np.mean(per_sample_avg))
            metrics["reasoning_tokens_per_turn/sample_std"] = float(np.std(per_sample_avg))
        metrics["assistant_turns/mean"] = float(np.mean([len(s) if s else 0 for s in rtpt]))
